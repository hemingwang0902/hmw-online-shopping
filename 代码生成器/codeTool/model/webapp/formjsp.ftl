...
<script type="text/javascript" src="${className}Form.js"></script>
</head>
<body>
...
<div class="container">
	<div class="content">
		...
		<form action="${packageName}Save.go" method="post" id="${packageName}Form">
			<s:token></s:token>
			<input type="hidden" id="ID" name="ID" value='<s:property value="ID"/>'/>
			<div class="lightbox_header"><span class="font_span">${tabCon}表单</span></div>
			<div class="btn_box">
				<input type="submit" value="保存" class="button_box"/>
				<input type="button" value="返回" onclick="location.href='${className}List.jsp';" class="button_box"/>
			</div>
