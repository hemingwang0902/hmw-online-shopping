...
$.extend($.validator, {
	...
	methods: {
				...
				success: function(result) {
					...
					if ( valid ) {
						...
						if(message==null||message==''){
							message = (previous.message = response || validator.defaultMessage( element, "remote" ));
						}else{
							previous.message=message;
						}
						//delete var message = response || validator.defaultMessage( element, "remote" );
		...
		validcharacter: function(value, element) {
		...
		equalSize: function(value, element, param) {
			// bind to the blur event of the target in order to revalidate whenever the target field is updated
			// TODO find a way to bind the event just once, avoiding the unbind-rebind overhead
			var target = $(param).unbind(".validate-equalSize").bind("blur.validate-equalSize", function() {
				$(element).valid();
			});
			var  reg=/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/;
			if(!reg.test(value)||!reg.test(target.val())){
				return true;
			}
			return parseInt(value) >= parseInt(target.val())?true:false;
		},
