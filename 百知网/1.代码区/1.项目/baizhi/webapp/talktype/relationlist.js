...
function getDataList(){
	...
	var TALKTYPE_ID = $("#TALKTYPE_ID").val();
	$.post("getRelationTalkList.go",{
...
function delData(ids){
	if(ids==null||ids==''){
		showmessage({message:"请选择设置话题信息",type:"info"});
		return ;
	}
	showmessage({message:"是否设置?",type:"error",callmethod:function(flag){
		if(flag){
			$.post("settingTalktype.go",{
				IDS:ids,
				TALKTYPE_ID:$("#TALKTYPE_ID").val(),
				TYPE:2
			},function(result){
				if(result==null||result==''){
					return;
