	...
	$("#ScoreLevelForm").validate({
		rules:{
			...
			SOCRE_DOWN: {
				required: true, 
				number:true,
				remote: {url: "checkScore.go",type: "post",dataType: "json",data: {
				//传递参数
		...
		messages:{
			NAME: {required: "请输入级别名称", rangelength: "长度为1至50"},
			SOCRE_DOWN: {required: "请输入积分下限",number:"积分下限必须为整数"},
			SOCRE_UP: {required: "请输入积分上限",number:"积分上限必须为整数",equalSize: "积分下限不能小于积分上限，请重新输入"},
