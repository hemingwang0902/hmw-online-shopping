...
public class IPSeeker {
	...
	private static final String DEFAULT_IPFILE_PATH = "template/qqwry.dat";
	 ...
	 */
	private IPSeeker() {
		if (StringUtils.isBlank(getIpFilePath())) {
			setIpFilePath(DEFAULT_IPFILE_PATH);
		}
	...
	private IPLocation getIPLocation(long offset) {
		try {
			...
			if (b == REDIRECT_MODE_1) {
				...
				ipLocation.setCountry(readString(readLong3()));
				ipLocation.setArea(readArea(offset + 8));
			} else {
	...
	private IPLocation getIPLocation(int offset) {
		...
		if (b == REDIRECT_MODE_1) {
			...
			if (b == REDIRECT_MODE_2) {
				...
				ipLocation.setCountry(readString(countryOffset));
			// 读取地区标志
			ipLocation.setArea(readArea(mbb.position()));
		} else if (b == REDIRECT_MODE_2) {
			ipLocation.setCountry(readString(readInt3()));
			ipLocation.setArea(readArea(offset + 8));
		} else {
			ipLocation.setCountry(readString(mbb.position() - 1));
			ipLocation.setArea(readArea(mbb.position()));
		}
		return ipLocation;
	}
	...
	public static void main(String[] args) {
		// 测试IP 124.225.67.149
		IPLocation l = IPSeeker.getInstance().getIPLocation("124.225.67.149");
		System.out.println(l.getCountry() + ":" + l.getArea());
	}
}