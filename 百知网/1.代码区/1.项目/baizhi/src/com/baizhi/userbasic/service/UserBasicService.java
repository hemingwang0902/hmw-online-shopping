package com.baizhi.userbasic.service;

import java.util.List;
import java.util.Map;
import org.dom4j.Element;
import com.baizhi.commons.ServiceSupport;
import com.baizhi.userbasic.dao.UserBasicDao;
/**
 * 
 * 类名：UserBasicService.java
 * 描述： 用户基本信息表服务类，负责增删改查
 * 创建者：江红
 * 创建日期： 2011-06-23 22:03:15
 * 版本：V0.9
 * 修改者：
 * 修改日期：
 */
public class UserBasicService extends ServiceSupport{
	
	private static final long serialVersionUID = -9216375503598824660L;
	
	private UserBasicDao userBasicDao;
	
	public UserBasicDao getUserBasicDao() {
		return userBasicDao;
	}

	public void setUserBasicDao(UserBasicDao userBasicDao) {
		this.userBasicDao = userBasicDao;
	}
	
	/**
	 * 新增或修改用户基本信息表信息
	 * 
	 * @param element  实体对象
	 * @return 返回主键ID,失败返回""
	 */
	public String saveOrUpdateUserBasic(Element element) {
		return userBasicDao.saveOrUpdateUserBasic(element);
	}
	
	/**
	 *　删除用户基本信息表信息
	 * 
	 * @param BASIC_IDS  用户基本信息表ID值集合以","分隔
	 * @return 返回boolean值,成功返回true,失败返回false
	 */
	public boolean deleteUserBasic(String BASIC_IDS) {
		return userBasicDao.deleteUserBasic(BASIC_IDS);
	}
	
	/**
	 * 根据用户基本信息表ID获取用户基本信息表实体
	 * @param BASIC_ID 用户基本信息表ID
	 * @return 返回用户基本信息表实体,如果无查询记录则返回null
	 */
	public  Element getUserBasicEleById(String BASIC_ID){
		return userBasicDao.getUserBasicEleById(BASIC_ID);
	}
	
	/**
	 *　获取用户基本信息表数量
	 * 
	 * @param params  参数
	 * @return 返回查询记录数量,失败返回-1
	 */
	public int getUserBasicCount(Map<String, Object> params) {
		return userBasicDao.getUserBasicCount(params);
	}
	
	/**
	 * 根据用户基本信息表ID获取用户基本信息表信息
	 * @param BASIC_ID 用户基本信息表ID
	 * @return 返回用户基本信息表信息,如果无查询记录则返回null
	 */
	public Map<String, Object> getUserBasicMapById(String BASIC_ID){
		return userBasicDao.getUserBasicMapById(BASIC_ID);
	}
	
	/**
	 * 获取用户基本信息表列表信息
	 * @param params 参数
	 * @param nowPage 当前页
	 * @param onePageCount 每页显示多少条
	 * @return 返回用户基本信息表列表信息,如果无查询记录则返回null
	 */
	public Map<String,Object> getUserBasicList(Map<String, Object> params,int nowPage,int onePageCount){
		return userBasicDao.getUserBasicList(params, nowPage, onePageCount);
	}
	
	/**
     * 获取用户基本信息表信息
     * 
     * @param  params 参数
     * @return 成功返回用户基本信息表信息,如果无查询记录则返回null
     */
	public List<Map<String,Object>> getUserBasicList(Map<String, Object> params){
		return userBasicDao.getUserBasicList(params);
	}

}
